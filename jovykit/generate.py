...
def write_generated_files(config: JovyConfig) -> None:
    ...
    shell_init = """# JovyKit shell bootstrap.
# Prefer image defaults when running in a container.
...
"""
    ...
    ssh_dir.chmod(0o700)
    bashrc_path = config.home_path / ".bashrc"
    if not bashrc_path.exists():
        bashrc_path.write_text(shell_init, encoding="utf-8")
