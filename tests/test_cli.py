...
def test_add_sync_regenerates_and_builds_when_requested(
    monkeypatch: pytest.MonkeyPatch, create_project: Any, run_cli: Any
) -> None:
    project = create_project()
    monkeypatch.chdir(project.root)
    ...
    monkeypatch.setattr(
        cli, "build_image", lambda config: built.append(config.image_ref)
    )

    run_cli(["add", "httpx", "--sync"])

    assert built == [project.config.image_ref]
    assert "httpx" in (project.env_dir / "requirements.txt").read_text(encoding="utf-8")


def test_sync_no_build_warns_without_building(
    monkeypatch: pytest.MonkeyPatch, create_project: Any, run_cli: Any
) -> None:
    project = create_project()
    monkeypatch.chdir(project.root)
...
def test_stop_stops_config_watcher_and_accepts_timeout(
    monkeypatch: pytest.MonkeyPatch, create_project: Any, run_cli: Any
) -> None:
    project = create_project()
    monkeypatch.chdir(project.root)
